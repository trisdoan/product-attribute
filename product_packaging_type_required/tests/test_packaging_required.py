...
class TestPackagingTypeRequired(common.TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.product = product = cls.env.ref("product.product_product_6")
        cls.pkg_type_model = cls.env["stock.package.type"]
        # create some required types
        cls.type_retail_box = cls.pkg_type_model.create(
            {"name": "Retail Box", "sequence": 3, "required": True}
        )
        cls.type_transport_box = cls.pkg_type_model.create(
            {"name": "Transport Box", "sequence": 4, "required": True}
        )
        cls.type_pallet = cls.pkg_type_model.create(
            {"name": "Pallet", "sequence": 5, "required": True}
        )
        ...
        cls.pkg_box = cls.env["product.packaging"].create(
            {
                ...
                "qty": 50,
                "package_type_id": cls.type_retail_box.id,
                "barcode": "BOX",
            }
        )

    @mute_logger(
        "odoo.addons.product_packaging_type_required.models.stock_package_type"
    )
    def test_cron_create(self):
        ...
        domain1 = [("package_type_id", "=", self.type_transport_box.id)]
        self.assertEqual(count_packaging(domain1), 0)
        domain2 = [("package_type_id", "=", self.type_pallet.id)]
        self.assertEqual(count_packaging(domain2), 0)
        domain3 = [("package_type_id", "=", self.type_retail_box.id)]
        ...
        self.pkg_type_model.create(
            {"name": "Small box", "sequence": 6, "required": True}
        )
        res = self.pkg_type_model.cron_check_create_required_packaging()
